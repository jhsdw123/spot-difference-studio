...
for (const r of results) {
  ...
  r.verdict = det.frac > 0.3 ? 'REJECT-global' : (det.exact ? 'OK' : `REVIEW-count-${det.regions.length}`);
  await reviewSheet(b, det.regions, `${dir}/${r.id}_review.png`);
  console.log(r.id, `count=${r.count} thr=${det.threshold} frac=${(r.frac * 100).toFixed(1)}% -> ${r.verdict}`);
}
