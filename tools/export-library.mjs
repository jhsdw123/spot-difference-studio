...
for (let from = 0; ; from += 100) {
  const r = await fetch(`${URL_BASE}/rest/v1/spotdiff_problems?select=id,image_a_url,image_b_url,diff_count,regions,image_width,image_height&is_active=eq.true&order=created_at.asc&limit=100&offset=${from}`, { headers });
...
for (const row of rows) {
  const entry = { id: row.id, count: row.diff_count, regions: row.regions, w: row.image_width, h: row.image_height };
  try {
